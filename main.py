...
def main():
    ...
    dice_one = app_functions.roll_die()
    dice_two = app_functions.roll_die()
    question_type = app_functions.get_question_type()
    app_functions.print_question(dice_one, dice_two, question_type)
    user_ans = app_functions.input_answer()
    if user_ans == -1:
        app_functions.print_error_message()
    is_correct = app_functions.is_correct_answer(dice_one, dice_two, question_type, user_ans)
    if is_correct:
        app_functions.print_congratulations(question_type)
    else:
        app_functions.print_correct_answer(dice_one, dice_two, question_type)
